...
export default function ClientWorkflowDetailPage() {
  ...
  const toggleItem = async (svcId: string, index: number, done: boolean) => {
    ...
    setBusy(true);
    try {
      const updated = await api.cwToggleCheck(wf._id, svcId, { index, done });
      setWf(updated);
    } catch { /* interceptor toasts */ }
    finally { setBusy(false); }
  };
  const completeService = async (svcId: string) => {
    if (!wf) return;
    if (!confirm('Mark this service done? Dependent services (if any) will unlock.')) return;
    setBusy(true);
    try {
      const updated = await api.cwCompleteService(wf._id, svcId);
      setWf(updated);
